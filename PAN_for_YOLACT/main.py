...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        loss, prec1 = train(train_loader, model, criterion, optimizer, epoch, log_training, tf_writer)
        losses_plot.append(loss)
        acc_plot.append(prec1)

        # evaluate on validation set
        if (epoch + 1) % args.eval_freq == 0 or epoch == args.epochs - 1:
            loss, prec1 = validate(val_loader, model, criterion, epoch, log_training, tf_writer)

            losses_plot_valid.append(loss)
            acc_plot_valid.append(prec1)
        ...
        print("Training for:", round((current_time - start)/60, 2), "minutes")

    epochs = range(0, args.epochs)
    plot_progress(epochs, losses_plot, losses_plot_valid, 'Loss', 'loss_progress_PA_8', label1='Training loss', label2='Validation loss')
    plot_progress(epochs, acc_plot, acc_plot_valid, 'Accuracy', 'accuracy_progress_PA_8', label1='Training accuracy', label2='Validation accuracy')
...
def plot_progress(epochs, y_data_train, y_data_valid, y_label, title, label1, label2):
    ...
    plt.ylabel(y_label)
    plt.legend()
    plt.savefig('visualisation/' + title)
