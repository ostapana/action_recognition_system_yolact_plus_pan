...
# may contain splits
parser.add_argument('--weights', type=str, default='checkpoint/8classes/PAN_Lite_somethingv2_resnet50_shift8_blockres_avg_segment8_e120/ckpt8.best8.pth.tar')
...
# for true test
parser.add_argument('--test_list', type=str, default= path_parent + '/yolact-camera/testset/valid8.txt')
parser.add_argument('--csv_file', type=str, default='output.csv')
...
def get_categories():
    with open(path_parent + '/yolact-camera/data/categories/categories8.txt') as f:
        categories = f.readlines()
    categories = sorted([i.strip() for i in categories])
    return categories
...
video_prob = [np.max(x[0]) for x in output]
...
if args.csv_file is not None:
    ...
    with open(test_file_list[0]) as f:
        content = f.readlines()
    vid_names = [n.split(' ')[0] for n in content]
    frames = [n.split(' ')[1] for n in content]
    print(len(vid_names))
    ...
    if args.dataset != 'somethingv2':  # only output top1
        ...
        with open(args.csv_file, 'w') as f:
            for n, fr, pred5, prob in zip(vid_names, frames, video_pred_top5, video_prob):
                fill = [n]
                fill.append(fr)
                fill.append(pred5[0])
                fill.append(str(round(prob, 3)))
                f.write('{} {} {} {} \n'.format(*fill))
...
for i in labels:
    ...
    pr.append((TP + TN) * 100 / (TP + FP + TN + FN))
...
df_cm = pd.DataFrame(cf / np.sum(cf) * 8, index=[i for i in categories],
...
plt.tight_layout()
plt.savefig('visualisation/8class/matrix8.png')
